...
class Question_paperController extends Controller
{
    public function list(){
        ...
        if(session()->get('admmin_is_super') == 'T'){
            ...
            $data['list'] = Question_paper::where('is_deleted',0)->orderBy('id','DESC')->get();
        }
    ...
    public function question(Request $request){
        ...
        $data['question_list'] = Question_detail::select(['question_id','question_text'])->whereNotIn('question_id',$question_ids)->where('language_id',1)->orderBy('id','DESC')->get();
    ...
    public function download_question(Request $request){
        ...
        foreach($data['subject_list'] as $key => $value){
            ...
            foreach ($value->question_list as $k1 => $v1) {
                ...
                array_push($question_list,$v1);
                if(($k1+1) % 8 == 0 && $k1 !== 0){
                    $data['question_list'][$indexA]['subject_name'] = $value->subject_name;
                    $data['question_list'][$indexA]['question_list_left'] = array_slice($question_list, 0, 4);
                    $data['question_list'][$indexA]['question_list_right'] = array_slice($question_list, 4, 8);
                    $indexA = $indexA + 1;
                    $question_list = [];
                }

                if(($k1+1) == count($value->question_list)){
        ...
        return $pdf->stream('exam_question.pdf');
    }
