...
@section('content')
    ...
    <div class="container-xl px-5">
        <div class="d-flex mt-10 mb-4 align-items-center">
            <h1 class="page-header mb-0">Edit Scholarship</h1>
        </div>
        <div class="row gx-5">
            <div class="col-lg-12">
                <form action="{{route('admin.scholarship.update')}}" method="post"  enctype="multipart/form-data">
                    <input type="hidden" name="id" id="id" value="{{$scholarship->id}}">
                    @csrf
                    <div class="row">
                        @foreach($languages as $language)
                            <div class="col-sm-12 alert alert-success" style="padding:30px;margin-top:20px;">
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <label for="Heading" class="custom-label">Scholarship Description</label>
                                            <div class="form-group">
                                                    <label for="name" class="col-sm-2 col-md-3 control-label"></label>
                                                    <div class="col-sm-12">
                                                        <textarea id="editor<?=$language->id?>" name="text_<?=$language->id?>"class="form-control" rows="5">
                                                            @php
                                                                $data = $scholarship->descriptions->firstWhere('language', $language->id);
                                                            @endphp
                                                            {{ $data ? $data->text : '' }}
                                                        </textarea>
                                                    </div>
                        ...
                        <div class="col-sm-12">
                            ...
                            <input class="form-control" id="video" type="text" name="video" placeholder=""  value="{{$scholarship->video}}" required/>
                        </div>
                        <div class="col-sm-12">
                            <label class="form-label" for="exampleFormControlInput">Status</label>
                            <div class="form-control" style="display:flex;">
                                <div class="form-check">
                                    <input class="form-check-input" id="status_active" type="radio" name="status" @if($scholarship->status == 1) checked  @endif value="1" />
                                    <label class="form-check-label" for="status_active">Active</label>
                                </div>
                                <div class="form-check" style="margin-left: 15px;">
                                    <input class="form-check-input" id="status_inactive" type="radio" name="status" @if($scholarship->status == 0) checked  @endif value="0" />
