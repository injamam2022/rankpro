                ...
                @if(count($list)>0)
                    @foreach ($list as $key=>$row)
                        <tr>
                            ...
                            <td>{{$row->total_right_answer}}</td>
                            <td>
                                <a href="{{route('admin.offline_exam.result_details',['exam_id'=>$row->exam_id,'id'=>$row->user_id])}}" class="btn btn-success btn-sm mr-1">View</a>
                            </td>
                        </tr>
                    @endforeach
                @endif
