    ...
    <div class="container-xl px-5">
        <div class="d-flex mt-10 mb-4" style="display:flex;">
            <div class="" style="width: calc(100% - 50px);">
                <h1 class="page-header mb-0">Online Exam</h1>
            </div>
            <div>
                <a href="{{route('admin.online_exam.add')}}" style="display: flex;font-size: 20px;text-decoration: none !important;">
                ...
                @if(count($list)>0)
                    @foreach ($list as $key=>$row)
                        <tr>
                            ...
                            <!-- <td>
                                <a href="{{route('admin.online_exam.question',['id'=>$row->id])}}" title="Click to change status" style="text-decoration: none;">
                                    <span class="btn btn-success btn-sm">View</span>
                                </a>
                            </td> -->
                            <!-- <td>
                                <a href="{{route('admin.online_exam.result',['id'=>$row->id])}}" title="Click to change status" style="text-decoration: none;">
                                    <span class="btn btn-success btn-sm">View</span>
                                </a>
                            </td> -->
                            <td>
                                <a href="{{route('admin.online_exam.change',['id'=>$row->id])}}" title="Click to change status" style="text-decoration: none;" onclick="return confirm('Do you realy want to change status?');">
                                    @if($row->status == 1)
